function m3ha_compare_neuronparams2 (paramValues, paramNames, suffices, varargin)
%% compare graphs across different sets of NEURON parameters
% Usage: m3ha_compare_neuronparams2 (paramValues, paramNames, suffices, varargin)
% Arguments:
...
%       suffices    - a cell array of corresponding suffices
...
% TODO: Plot the different conditions (suffices) against each other in the same graphs
%       May need to make version 2s for all the compute_and_plot functions that will
%       do this when suffices is passed as a cell array
%
...
addRequired(iP, 'suffices', ...         % corresponding suffices
    @(x) assert(iscell(x) && (min(cellfun(@ischar, x)) ...
        ...
        'suffices must be a cell array of strings/character arrays!'));
...
parse(iP, paramValues, paramNames, suffices, varargin{:});
v = iP.Results.v;
...
if max(cellfun(@(x) ~isempty(find_in_strings(x, changedNames, ...
    ...
    if numel(suffices) == 2
        ...
        if ~isempty(commonStrs) && numel(commonStrs) == 2
            ...
            sufficesSimplified = ...
                {suffices{1}, strrep(suffices{2}, ['_', commonStrs{2}], '')};
        else
            sufficesSimplified = suffices;
        end

        % Plot invidual plots
        m3ha_compute_and_plot_geometry(outFolder, suffices{1}, ...
                                   diamSoma(1), LDend1(1), LDend2(1), 'b', 2);
        m3ha_compute_and_plot_geometry(outFolder, suffices{2}, ...
                                   diamSoma(2), LDend1(2), LDend2(2), 'r', 2); 
    else
        sufficesSimplified = suffices;
    end
    m3ha_compute_and_plot_geometry2(outFolder, sufficesSimplified, ...
...
if max(cellfun(@(x) ~isempty(find_in_strings(x, changedNames, ...
    ...
    m3ha_compute_and_plot_IT2(v, 'OutFolder', outFolder, ...
                        'Suffices', suffices, 'Celsius', celsius, ...
                        'Cout', caOut, 'Cin', caIn, ...
                        'Pcabar', pcabarITSoma, ...
...
if max(cellfun(@(x) ~isempty(find_in_strings(x, changedNames, ...
    ...
    m3ha_compute_and_plot_all_IV2(v, 'OutFolder', outFolder, ...
                            'Suffices', cellfun(@(x) [x '_soma'], suffices, ...
                                                'UniformOutput', false), ...
    ...
    m3ha_compute_and_plot_all_IV2(v, 'OutFolder', outFolder, ...
                            'Suffices', cellfun(@(x) [x '_dend1'], suffices, ...
                                                'UniformOutput', false), ...
                            'Celsius', celsius, 'Ek', eK, 'ENa', eNa, ...
                            'Eh', eh, 'CaOut', caOut, 'CaIn', caIn, ...
    ...
    m3ha_compute_and_plot_all_IV2(v, 'OutFolder', outFolder, ...
                            'Suffices', cellfun(@(x) [x '_dend2'], suffices, ...
                                                'UniformOutput', false), ...
                            'Celsius', celsius, 'Ek', eK, 'ENa', eNa, ...
                            'Eh', eh, 'CaOut', caOut, 'CaIn', caIn, ...
...
m3ha_compute_and_plot_geometry2(outFolder, suffices, ...
