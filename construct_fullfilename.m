...
% Outputs:	fullfilename 	- the full path to file constructed
% Arguments:	filename 	- filename with extension but without directory, e.g. 'A100110_0008_18.mat'
%				must be a string scalar or a character vector
% 		varargin	- 'Directory': a full directory path, e.g. '/media/shareX/share/'
%			 	must be a string scalar or a character vector
%				default == pwd
...
% 2017-05-04 Changed paramname, paramvalue -> 'NameValuePairs' and made it a parameter 
...
if nargin < 1
	error('Not enough input arguments, type ''help construct_fullfilename'' for usage');
end

% Add required inputs to an input Parser
iP = inputParser;
addRequired(iP, 'filename', ...				% filename with extension but without directory, e.g. 'A100110_0008_18.mat'
	@(x) validateattributes(x, {'char', 'string'}, {'nonempty'}));
%	@(x) validateattributes(x, {'char', 'string'}, {'scalartext'}));	% introduced after 2016B

% Add parameter-value pairs to the input Parser
addParameter(iP, 'Directory', '', ...			% a full directory path, e.g. '/media/shareX/share/'
	@(x) validateattributes(x, {'char', 'string'}, {'nonempty'}));
...
final_suffix = construct_suffix('Suffices', suffices, 'NameValuePairs', namevaluepairs);

%% Construct full file name
if isempty(final_suffix)			% if nothing provided
	% Construct path based on directory, filebase and extension
	fullfilename = fullfile(directory, [filebase, extension]);	
elseif ~isempty(final_suffix)			% suffix(ces) is(are) provided
	% Construct path based on directory, filebase, final suffix and extension
	fullfilename = fullfile(directory, [filebase, '_', final_suffix, extension]);
end
...
fullfilename = construct_fullfilename(ntrials, directory, filename, paramname, paramvalue)

if ntrials == 1
	fullfilename = fullfile(directory, filename);
else
	fullfilename = fullfile(directory, ...
			strrep(filename, '.', ['_', paramname, ...
			'_', num2str(paramvalue), '.']));
end

if strcmp(directory, 'nodir')
if isequal(suffices, 'nosuffices') && strcmp(namevaluepairs{1}, 'nopairs')	% if nothing provided
