...
if plotFlag
    ...
    if plotIndividualFlag
        ...
        figTitle = sprintf('All traces for Experiment %s', ...
                            strrep(prefix, '_', '\_'));
    ...
    if plotOverlappedFlag
        ...
        yLabelsOverlapped = {'Rec V (mV)', 'Sim V_{soma} (mV)', ...
                            'Sim V_{dend1} (mV)', 'Sim V_{dend2} (mV)'};
...
for iSwp = 1:length(ltsswperr)
    ...
    elseif ~isnan(real_ltsdata(iSwp)) && ~isnan(sim_ltsdata(iSwp))    % LTS does exist in both cases
        % Compute dimensionless LTS error by normalizing with uncertainty in measurement
