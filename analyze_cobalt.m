...
lowpassNpoles = 4;                   	% order of the ButterWorth filter
...
lowCutoffs = [0, 300];
highCutoffs = [300, 4000];
bandName = {'0to300', '300to4000'};

signalLabel = 'Voltage (uV)';
windowSizeSeconds = 4;          % window size in seconds
windowIntervalSeconds = 0.5;    % window interval in seconds
maxLagSeconds = Inf;            % maximum lag in seconds
normalization = 'none';         % normalization method for xcorr
channelToPlot1 = 2;             % the first channel number to plot
channelToPlot2 = 3;             % the second channel number to plot
selectMethod = 'consecutive';   % selection method for pairs to plot
...
% Save data
dataFileName = fullfile(outFolder, [fileBase, '.mat']);
...
if crossCorrFlag
    if bandpassFilterFlag
        for iBand = 1:nBands
            lowFreq = lowCutoffs(iBand);
            highFreq = highCutoffs(iBand);
            fileBaseCorr = [fileBase, ' band', num2str(lowCutoffs(iBand)), ...
                                      'to', num2str(highCutoffs(iBand))];

            [corrProf, lagProf, corrMatrix, lagMatrix] = ...
                crosscorr_profile(dataRaw{iBand}, ...
                        'SamplingRateHz', samplingRate, ...
                        'OutFolder', outFolder, 'FileBase', fileBaseCorr, ...
                        'LowFreq', lowFreq, 'HighFreq', highFreq, ...
                        'DataFilt', dataFiltered{iBand}, ...
                        'WindowSizeSeconds', windowSizeSeconds, ...
                        'WindowIntervalSeconds', windowIntervalSeconds, ...
                        'MaxLagSeconds', maxLagSeconds, ...
                        'Normalization', normalization, ...
                        'ChannelToPlot1', channelToPlot1, ...
                        'ChannelToPlot2', channelToPlot2, ...
                        'SelectMethod', selectMethod, ...
                        'SignalLabel', signalLabel);
        end
    else
        [corrProf, lagProf, corrMatrix, lagMatrix] = ...
            crosscorr_profile(dataRaw, 'SamplingRateHz', samplingRate, ...
