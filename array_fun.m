...
if is_in_parallel || numel(varargin{1}) <= minItemsForParfor
    ...
    % Force all arguments as column cell vectors or dimension nItems x 1
    inputColumn = array_fun(@force_column_cell_individually, inputList, ...
                            'UniformOutput', false);
    ...
    % Reorganize outputs
    outputCells = array_fun(@(x) outputMatrix(:, x), 1:nArgOut, ...
                            'UniformOutput', false);
    ...
    if oldDimensions(2) ~= 1
        outputCells = array_fun(@(x) reshape(x, oldDimensions), ...
    ...
    if isfield(params, 'UniformOutput') && ~params.UniformOutput || ...
        ...
        varargout = array_fun(@cell2num, outputCells, ...
                                'UniformOutput', false);
    end
end
...
array = force_column_vector(array, 'TreatCellAsArray', true);
