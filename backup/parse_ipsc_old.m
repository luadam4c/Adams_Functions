function [peakTimeMs, idxPeak, peakAmplitude, peakDelay] = parse_ipsc (tvec0, ivec0s, istart, ipeakwin, plotflag, outfolder, filebase)
%% Finds time of current peak from a an inhibitory current trace (must be negative current)
% Usage: [peakTimeMs, idxPeak, peakAmplitude, peakDelay] = parse_ipsc (tvec0, ivec0s, istart, ipeakwin, plotflag, outfolder, filebase)
% Explanation:
%       TODO
%
% Examples:
...
%                           idxPeak         - index of ipeak relative to ivec0s (a row vector)
%                           peakDelay       - delay of ipeak (peakTimeMs - istart) in ms (a row vector)
%                           peakTimeMs      - time of ipeak (tvec0(idxPeak)) in ms (a row vector)
%                           peakAmplitude   - amplitude of ipeak in pA (a row vector)
%
% Arguments:
...
%% Check arguments
if nargin < 2
    error('Not enough input arguments, type ''help parse_ipsc'' for usage');
...
peakAmplitude = zeros(1, nswps);            % amplitude of ipeak in pA
idxPeak = zeros(1, nswps);            % index of ipeak relative to ivec0s
parfor swp = 1:nswps                % FOR each sweep
    [vtemp, itemp] = min(ivec0s_part(:, swp));
    peakAmplitude(swp) = vtemp;                % peakAmplitude is the amplitude of ipeak
    idxPeak(swp) = (ind_begin - 1) + itemp;    % index of ipeak relative to ivec0s
    % fprintf('Current peak found at index %g with amplitude = %g pA\n\n', idxPeak(swp), peakAmplitude(swp));
end
peakTimeMs = tvec0(idxPeak)';            % time of ipeak in ms
peakDelay = peakTimeMs - istart;        % delay of ipeak (peakTimeMs - istart) in ms
...
if plotflag == 1
    ...
    for swp = 1:nswps            % Plot each current trace and mark peak amplitude
        ...
        plot(tvec0(idxPeak(swp)), peakAmplitude(swp), 'Marker', 'x', 'MarkerSize', 12);
    end
    xlabel('Time (ms)')
    ylabel('Current (pA)')
...
ind = IPSC_ind:round(ipscpwin(2)/sims);     % indices of interest   % NOT ROBUST
    ...
    idxPeak = (ind(1) - 1) + itemp1;      % index of ipeak relative to ivec0s
    peakTimeMs = tvec0(idxPeak);          % time of ipeak in ms
