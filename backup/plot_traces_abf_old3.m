function [d, sius, tvec] = plot_traces_abf (filename, expmode, recmode, outfolder, left, right, plotmode, t_units, ch_units, ch_labels)
%% Takes an abf file and plots all traces
% Usage: [d, sius, tvec] = plot_traces_abf (filename, expmode, recmode, outfolder, left, right, plotmode, t_units, ch_units, ch_labels)
% Outputs:
%	d		- full data
...
% 2017-03-15 - BT - adapted labelling detection for conductance and recmodes (see above)
...
%%% TODO: complete this for all arguments
elseif nargin >= 4 && ~isdir(outfolder)
	error('outfolder must be a directory!');
elseif nargin >= 7 && plotmode ~= 1 && plotmode ~= 2
	error('The plot mode %d is not currently supported!', plotmode);
elseif nargin >= 8 && ~ischar(t_units)
...
if nargin < 10 || isempty(ch_labels)
	if strcmp(expmode, 'EEG')
...
% Set default xlimits
if nargin < 5 || isempty(left)
	left = 0;
end
if nargin < 6 || isempty(right)
	right = tvec(end);
end
...
if ndim == 2 && plotmode == 1		% Usually EEG
	if strcmp(expmode, 'EEG')
		...
		vvec_all = d;
		figure_num = 1;
		h = plot_traces_abf_helper(figure_num, tvec, vvec_all, left, right, t_units, nchannels);
	...
	elseif strcmp(expmode, 'patch')
		...
		for j = 1:nchannels
			...
			vec_all = squeeze(d(:, j, :));
			figure_num = 1*j;
			h = plot_traces_abf_helper(figure_num, tvec, vec_all, left, right, t_units, 1);
			title(sprintf('Data for all channels between %.1f %s and %.1f %s', left, t_units, right, t_units));
...
elseif ndim == 3 && plotmode == 1	% Usually Patch clamp
	...
	for j = 1:nchannels
		...
		vec_all = squeeze(d(:, j, :));
		figure_num = 100*j;
		h = plot_traces_abf_helper(figure_num, tvec, vec_all, left, right, t_units, nsweeps);
...
if ndim == 2 && plotmode == 2		% Usually EEG
	if strcmp(expmode, 'EEG')
		for j = 1:nchannels
			...
			vvec = d(:, j);
			figure_num = 1+j;
			h = plot_traces_abf_helper(figure_num, tvec, vvec, left, right, t_units, 1);
	...
	if strcmp(expmode, 'EEG')
		for j = 1:nchannels
			for k = 1:nsweeps
				...
				vec = d(:, j, k);
				figure_num = 100*j+k;
				h = plot_traces_abf_helper(figure_num, tvec, vec, left, right, t_units, 1);
...
function [h] = plot_traces_abf_helper(fig_num, tvec, vvec, left, right, t_units, channels_or_sweep_num)
%% Plots traces, sets axes and xlabel
% Usage: [h] = plot_traces_abf_helper(fig_num, tvec, vvec, left, right, t_units, channels_or_sweep_num)
%	h			- figure
