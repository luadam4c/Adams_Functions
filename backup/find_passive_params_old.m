...
sc_max = 10;		% maximum scaling factor from initial voltage amplitude value
...
rmse_R = measure_error(xvecRs, yvecRs, eqform_R, dv_rec_mean, sc_max, typtau0_R, typtau1_R, tau0_range_R, tau1_range_R, pw_mean);
rmse_F = measure_error(xvecFs, yvecFs, eqform_F, dv_rec_mean, sc_max, typtau0, typtau1, tau0_range, tau1_range, pw_mean);
...
if ~isempty(xvecR_all)
	...
	[cfit_R1, eqnS_R1, eqnL_R1, CS0_R1, tau0_R1, CS1_R1, tau1_R1, ~, ~] = ...
		FitData ('rising', eqform_R, xvecR_all, yvecR_all, dv_rec_mean, sc_max, typtau0_R, typtau1_R, tau0_range_R, tau1_range_R, pw_mean);
...
if ~isempty(xvecF_all)
	...
	[cfit_F1, eqnS_F1, eqnL_F1, ~, tau0_F1, ~, tau1_F1, CL0_F1, CL1_F1] = ...
		FitData ('falling', eqform_F, xvecF_all, yvecF_all, dv_rec_mean, sc_max, typtau0, typtau1, tau0_range, tau1_range, pw_mean);
...
if ~isempty(xvecR_mean)
	...
	[cfit_R2, eqnS_R2, eqnL_R2, CS0_R2, tau0_R2, CS1_R2, tau1_R2, ~, ~] = ...
		FitData ('rising', eqform_R, xvecR_mean, yvecR_mean, dv_rec_mean, sc_max, typtau0_R, typtau1_R, tau0_range_R, tau1_range_R, pw_mean);
...
function [ft, coeff_start, coeff_lower, coeff_upper] = fit_setup(eqform, C_Est, sc_max, typtau0, typtau1, tau0_range, tau1_range)
...
function rmse_swp = measure_error(xvecs, yvecs, eqform, C_Est, sc_max, typtau0, typtau1, tau0_range, tau1_range, pulse_width);
%% Returns root-mean-squared error for each trace fitted individually

% Setup fitting type, initial conditions and boundaries
[ft, coeff_start, coeff_lower, coeff_upper] = fit_setup(eqform, C_Est, sc_max, typtau0, typtau1, tau0_range, tau1_range);
...
function [cfit, eqnS, eqnL, CS0, tau0, CS1, tau1, CL0, CL1] = ...
	FitData (phase, eqform, xvec, yvec, C_Est, sc_max, typtau0, typtau1, tau0_range, tau1_range, pulse_width)
...
if strcmp(phase, 'falling')
	[CL0, CL1] = LongFromShort (CS0, tau0, CS1, tau1, pulse_width);
...
function [CL0, CL1] = LongFromShort (CS0, tau0, CS1, tau1, pulse_width)
%% Estimate the long pulse response coefficients from the short pulse response coefficients
% See equation 4.5.56 on p. 95 of Johnston % Wu

CL0 = CS0 / (1 - exp(-pulse_width/tau0));
CL1 = CS1 / (1 - exp(-pulse_width/tau1));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [params] = ParamsFromCoeff (C0, tau0, C1, tau1, cpa_mean, Cm, Ra, Rs)
...
function plot_cfit (phase, win, cfit, xvec, yvec, params, eqnS, eqnL, tau0, tau1, CL0, CL1, pulse_width)
...
if strcmp(phase, 'falling')
	...
	paddedt0 = -log(1 - exp(-pulse_width/tau0))*tau0;
	% fprintf('paddedt0 == %g\n', paddedt0);
	paddedt1 = -log(1 - exp(-pulse_width/tau1))*tau1;
	% fprintf('paddedt1 == %g\n', paddedt1);
