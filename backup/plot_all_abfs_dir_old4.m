...
addpath_custom(fullfile(functionsdirectory, '/Adams_Functions/'));		% for identify_channels.m
...
parfor k = 1:nfiles
	...
	if length(current_data) > 20000
		...
		reduction = abs(diff(diff(avgs_byswp)));			% reduces sweeps into differences between successive sweep averages
