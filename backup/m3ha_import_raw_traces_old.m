function [data, sweepInfo, dataCprAll] = ...
...
timeToPadDefault = 0;
responseWindowDefault = [];
stimStartWindowDefault = [];
...
% TODO: Fix initial_slopes to output a lookup table and use it here
    ...
    vVecs = cellfun(@(x, y, z) ...
                    ...
                    num2cell(isOutOfBalance), vVecs, iVecs, ...
                    'UniformOutput', false);
end
...
vvecsToPad = arrayfun(@(x) NaN * ones(x, 1), nSamplesToPad, ...
                        'UniformOutput', false);
ivecsToPad = arrayfun(@(x) zeros(x, 1), nSamplesToPad, ...
                        'UniformOutput', false);
gvecsToPad = arrayfun(@(x) zeros(x, 1), nSamplesToPad, ...
                        'UniformOutput', false);
...
tVecs = create_time_vectors(nSamples, 'TimeUnits', 'ms', ...
...
if toParsePulse && toAverageByVhold
    ...
    currentPulseAmplitude = mean(pulseAmplitude) * ones(nSwps, 1);
else
    data = data;
    fileNames = fileNames;
    currentPulseAmplitude = pulseAmplitude;
end
...
figName = fullfile(outparams.outFolder, ...
                    [outparams.prefix, ...
...
% Find the indices for the approximate current pulse response
endPointsApproxCpr = find_window_endpoints(approxWindowOrig, tVecs);
...
% Compute the index to start each response vector
%   Note: this might be less than 1
idxStart = idxStimStart - baseLengthSamples;

% Compute the index to end each response vector
idxEnd = idxStimStart + responseLengthSamples - 1;

baseLengthMs = ipscTimeOrig;
idxStimStartMs = ipscTimeOrig;
responseLengthMs = ipscDur;
