function parse_current_family (filename, alldata, sius, outfolder)
%% From a current injection protocol, detect spikes for each sweep and make an F-I plot
% Usage: parse_current_family (filename, alldata, sius, outfolder)
% Arguments:
...
addpath_custom(fullfile(functionsdirectory, '/Adams_Functions/'));	% for construct_abffilename.m
...
parfor i = 1:nsweeps		%%% TODO: See if parfor works
%for i = 1:nsweeps
	cdata = alldata(:,1,i);
	spike_indices = find(is_spike(:,1,i));
	spi_within_injection = spike_indices(spike_indices > 11500 & spike_indices < 21600);
	...
	if length(spi_within_injection) > 1
		...
		spike_freq(i) = (length(spi_within_injection) - 1) / ...
		 		((timepoints(spi_within_injection(end)) - timepoints(spi_within_injection(1))) * 1000);
	else
		spike_freq(i) = 0;
	end
end
...
for i = 1:nsweeps
	...
	y = ylim;
	line([timepoints(11500) timepoints(11500)], [y(1) y(2)]);
	line([timepoints(21600) timepoints(21600)], [y(1) y(2)]);
	plot(timepoints(spike_indices(spike_indices > 11500 & spike_indices < 21600)), cdata(spike_indices(spike_indices > 11500 & spike_indices < 21600)), 'xg');
	if length(spike_indices) > 1
