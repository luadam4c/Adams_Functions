function [tVecGabab, iVecGabab, vVecStim, features, h] = ...
                compute_and_plot_gabab (fileName, varargin)
%% Computes and plots an evoked GABA-B IPSC with its stimulus
% Usage: [tVecGabab, iVecGabab, vVecStim, features, h] = ...
%               compute_and_plot_gabab (fileName, varargin)
% Explanation:
%       TODO
% Example(s):
%       [tVecGabab, iVecGabab, vVecStim, features, h] = ...
%           compute_and_plot_gabab('20180914C_0001');
% Outputs:
%       tVecGabab   - time vector for evoked GABA-B IPSC
%                   specified as a numeric column vector
%       iVecGabab   - voltage trace of evoked GABA-B IPSC
%                   specified as a numeric column vector
%       vVecStim    - current trace of stimulation voltage pulse
%                   specified as a numeric column vector
%       features    - computed GABA-B IPSC features, a table with fields
%                       peakAmplitude
%                       peakSlope
%       h           - handle to figure
...
tVecGabab = tVec(idxCprStartFirst:idxCprEndFirst);
...
vVecStim = mean(iVecCprs, 2);

% Average the current pulse responses to get the evoked GABA-B IPSC
iVecGabab = mean(vVecCprs, 2);
...
baseValue = mean(iVecGabab(1:baselineLengthSamples));
...
[peakValue, temp1] = max(iVecGabab((idxResponseEnd + 1):end));
