function m3ha_compare_neuronparams (paramValues, paramNames, suffices, varargin)
%% compare graphs across different sets of NEURON parameters
% Usage: m3ha_compare_neuronparams (paramValues, paramNames, suffices, varargin)
% Arguments:
...
if max(cellfun(@(x) ~isempty(find_in_strings(x, changedNames, ...
    ...
    for iSet = 1:nSets      % for each set of parameters
        ...
        % Plot geometry
        m3ha_compute_and_plot_geometry(outFolder, suffices{iSet}, ...
...
if max(cellfun(@(x) ~isempty(find_in_strings(x, changedNames, ...
    ...
    for iSet = 1:nSets      % for each set of parameters
        ...
        m3ha_compute_and_plot_IT(v, 'OutFolder', outFolder, ...
                            'Suffix', suffices{iSet}, 'Celsius', celsius, ...
                            'Cout', caOut, 'Cin', caIn, ...
                            'Pcabar', pcabarITSoma, ...
