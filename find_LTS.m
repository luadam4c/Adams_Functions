function [actVhold, maxnoise, peaktime, peak2ndder, peakprom, peakwidth, peakclass, spikesperpeak, ltspeaktime, ltspeakval, maxslopetime, maxslopeval, bursttime, spikesperburst, maxspikeamp, minspikeamp, spikefrequency, spikeadaptation, LTScouldbemissed] = find_LTS (tvec0, vvec0, istart, ipeakt, hrangeORmaxnoise, ltswin, plotflag, outfolder, filebase, tvec2, vvec1, vvec2, vvec3)
...
dvvec3 = diff(vvec3)./diff(tvec0);		% differentiate vvec3, the median-filtered then smoothed voltage trace
...
if (~isempty(ptemp3) && isspontaneous ~= 1) ...	% either peakclass hasn't been classified yet
	...
	sp_thr = ltspeakval + sp_thr_rel_lts;	% Spike threshold (mV)

	% Find max slope based off of v3
...
if isnan(ltspeaktime)				% must be an "LTS" to begin with
	...
	if isempty(allspi)			% no spikes detected, not a "burst"
		...
		if ~ismember(filebase, traces_to_override{ind_Spbi}) ...
			...
			v0_pk_begin(psel3) = vpeak_lb(psel3);
		...
		if ~isempty(spi{psel3})
			...
			% Convert index in terms of vvec0
			if isempty(ind8)
				bon_i = (v_begin - 1) + v0_pk_begin(psel3);
			else
				bon_i = (v_begin - 1) + ind8;
			end
...
%			'Color', 'b');								% line for maxslope
		...
		legend('raw trace', 'median-filtered', 'median-filtered then moving-average-filtered', ...
			peakclass_label, peakfeature_label, 'burst onset', 'spikes', ...
...
%	close(h);
	...
	line(xlimits, [lts_thr lts_thr], 'Color', 'r', 'LineStyle', '--', 'LineWidth', 0.5);		% mark LTS threshold
	line(xlimits, [lts_thr_alt lts_thr_alt], 'Color', 'b', 'LineStyle', '--', 'LineWidth', 0.5);	% mark alt LTS threshold
	xlim(xlimits);
	ax = gca;
	ylimits = get(ax, 'YLim');
