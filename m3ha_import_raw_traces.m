...
vvecsToPad = array_fun(@(x) NaN * ones(x, 1), nSamplesToPadTotal, ...
                        'UniformOutput', false);
ivecsToPad = array_fun(@(x) zeros(x, 1), nSamplesToPadTotal, ...
                        'UniformOutput', false);
gvecsToPad = array_fun(@(x) zeros(x, 1), nSamplesToPadTotal, ...
                        'UniformOutput', false);
...
vVecs = array_fun(@(x, y) vertcat(x, y), vvecsToPad, vVecsExtracted, ...
                    'UniformOutput', false);
iVecs = array_fun(@(x, y) vertcat(x, y), ivecsToPad, iVecsExtracted, ...
                    'UniformOutput', false);
gVecs = array_fun(@(x, y) vertcat(x, y), gvecsToPad, gVecsExtracted, ...
                    'UniformOutput', false);
...
nSamples = array_fun(@length, vVecs);
...
data = array_fun(@(x, y, z, w) horzcat(x, y, z, w), ...
...
fileNames = array_fun(@(x) strcat(fileNames{1}(1:7), ...
