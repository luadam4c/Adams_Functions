function hout = suptitle_custom (str)
...
%   2020-08-06 Changed titleypos from .95 to .98
...
for i=1:numAxes
    pos=get(h(i),'pos');
    thePositions(i,:) = pos;
    if ~strcmp(get(h(i),'Tag'),'suptitle_custom')
        if pos(2) < min_y
            min_y=pos(2)-ff/5*3;
        end
...
axes('pos',[0 1 1 1],'visible','off','Tag','suptitle_custom');
