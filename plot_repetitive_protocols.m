...
if isempty(abfParamsCell) || isempty(abfDataCell)
    ...
    % Get the table
    abfParamsStruct = [abfParamsCell{:}];
    abfParamsTable = struct2table(abfParamsStruct);
end
    ...
    if isProtocol
        ...
        [tVecAvg, respAvg, stimAvg, featuresAvg, h1] = ...
            compute_and_plot_average_response(fileName, responseType, ...
                'Verbose', verbose, 'OutFolder', outFolderProtocol, ...
...
%     plot_table(featuresFileTable, 'VarsToPlot', varToPlot, ...
...
% 
%     % Plot each variable of the table
%     plot_table(featuresSweepTable, 'VarsToPlot', varToPlot, ...
