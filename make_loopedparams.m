...
% Outputs:    
%       pchnames    - a cell array of parameter names or ordered pairs of parameter names for each trial
%       pchvalues   - a numeric array of parameter values 
%                    or a cell array of ordered paris of parameter values for each trial
...
%                       'cross'    - Loop through each parameter while fixing others
%                       'grid'    - Loop through all possible combinations of parameters
...
%                   must be a numeric vector
%       varargin    - 'OutFolder': directory to place outputs, e.g. '/media/shareX/share/'
...
if nargin < 7
    error('Not enough input arguments, type ''help make_loopedparams'' for usage');
end

% Add required inputs to an input Parser
iP = inputParser;
addRequired(iP, 'loopmode', ...         % how to loop through parameters: 'cross' or 'grid'
    @(x) any(validatestring(x, {'cross', 'grid'})));
...
addParameter(iP, 'OutFolder', '', ...   % directory to place outputs, e.g. '/media/shareX/share/'
    @(x) validateattributes(x, {'char', 'string'}, {'nonempty'}));
%    @(x) validateattributes(x, {'char', 'string'}, {'scalartext'}));    % introduced after 2016B
addParameter(iP, 'FileLabel', datestr(clock, 30), ...    % label for output files
    @(x) validateattributes(x, {'char', 'string'}, {'nonempty'}));
%    @(x) validateattributes(x, {'char', 'string'}, {'scalartext'}));    % introduced after 2016B
...
loopmode = validatestring(loopmode, {'cross', 'grid'});
...
if numel(unique([numel(pnames), numel(plabels), numel(pislog), numel(pmin), numel(pmax), numel(pinc)])) ~= 1
...
switch loopmode
case 'cross'
    ntrials = sum(nperp);        % total number of trials in 'cross' mode
case 'grid'
    ntrials = prod(nperp);        % total number of trials in 'grid' mode
end
...
case 'cross'
    ...
    for p = 1:nump
        % Store parameter names and values for each trial (to keep things in order)
        indices = (ct + 1):(ct + nperp(p));                % indices for this parameter
        pchnames(indices) = repmat(pnames(p), 1, length(indices));    % parameter name for each trial
